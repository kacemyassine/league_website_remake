...
export function TopScorers({ onEditPlayer, hideButtons = false, players: propPlayers, teams: propTeams }: TopScorersProps) {
  const storePlayers = useLeagueStore(s => s.players);
  const storeTeams = useLeagueStore(s => s.teams);
  const deletePlayer = useLeagueStore(s => s.deletePlayer);
              ...
              return (
                ...
                >
                  ...
                  <div className="text-right shrink-0">
                    <p className="text-xl md:text-2xl font-bold text-secondary">{player.goals || 0}</p>
